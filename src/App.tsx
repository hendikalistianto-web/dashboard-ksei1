...
export default function App() {
  ...
  // ANTI-LAG SEARCH DEBOUNCE (Waktu tunda sedikit dinaikkan untuk performa)
  useEffect(() => {
  ...
  // CLOUD DATABASE FETCH DEFAULT DATA
  useEffect(() => {
    if (!user || !db) return;
  ...
  // PROSES DAN SAVE KE CLOUD DATABASE DENGAN BATCH ROBUST
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (user && db) {
          ...
          // 2. Tulis data baru (Tanpa stringify, data sudah dijamin aman tanpa undefined dari processData)
          const chunkSize = 400; 
          let writeBatches = [];
          ...
          // Fallback lokal jika database tidak terhubung
          setData(mappedData);
          setIsUploaded(true);
        }
      } catch (error) {
        console.error("Save to Cloud DB Error:", error);
    ...
    if (isFetchingDB) {
       return (
         <div className="flex flex-col items-center justify-center py-32 opacity-70 h-full w-full">
            ...
            <p className="text-blue-400 font-bold text-center text-lg">Menghubungkan ke Cloud...<br/>
              <span className="text-sm font-normal text-slate-500 mt-2 inline-block">Mencari data kolaborasi terakhir</span>
            </p>
         </div>
       );
    }
    if (!isUploaded && data.length === 0) return (
      <div className="mb-6 bg-blue-900/20 border border-blue-500/30 rounded-2xl p-6 text-center lg:text-left flex flex-col lg:flex-row items-center justify-between">
        <div>
          <h3 className="text-blue-400 font-bold text-lg mb-2">Sistem Siap Digunakan</h3>
          <p className="text-sm text-slate-400 max-w-2xl">
            Sistem tidak menemukan data default di Cloud. Silakan klik tombol <b>"Upload Admin CSV"</b> untuk menanamkan database baru KSEI (&ge;1%) ke server.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] text-slate-300 font-sans selection:bg-blue-900 selection:text-blue-100 flex flex-col md:flex-row overflow-hidden relative">
      
      {/* OVERLAY LOADING MENYIMPAN KE CLOUD */}
      {isSaving && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-4">
           ...
           <h2 className="text-xl text-white font-bold tracking-wider">Menyimpan ke Cloud Database...</h2>
           <p className="text-slate-400 text-sm mt-2 text-center max-w-sm leading-relaxed">
             Proses ini akan merekap ribuan baris data CSV secara permanen. Mohon jangan tutup layar atau memuat ulang halaman.
           </p>
        </div>
      )}
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#151e2f] border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="py-4">
               <p className="text-sm text-slate-400 mb-4 leading-relaxed">
                 Hanya owner/admin yang memiliki otorisasi untuk mengubah data <i>Cloud Default</i> KSEI ini. Masukkan sandi untuk melanjutkan.
               </p>
               <input 
                 type="password"
                 value={passwordInput}
                 ...
                 autoFocus
               />
               {passwordError && <p className="text-rose-500 text-xs font-semibold mt-2 animate-pulse">Sandi salah. Coba lagi atau hubungi owner.</p>}
            </div>
            <div className="flex justify-end gap-3 mt-2">
              ...
              <button onClick={handlePasswordSubmit} className="px-5 py-2.5 rounded-xl text-sm font-bold bg-blue-600 hover:bg-blue-500 text-white transition-colors shadow-lg shadow-blue-900/30">Akses & Upload</button>
            </div>
          </div>
        </div>
      )}
      ...
      <aside className="w-full md:w-64 bg-[#111827] border-b md:border-b-0 md:border-r border-slate-800 flex flex-col z-20 shrink-0">
        <div className="p-6 flex flex-col items-start gap-4 border-b border-slate-800/50">
          <div className="bg-white p-2 rounded-xl shadow-lg shadow-black/20 w-full flex items-center justify-center">
            <img 
              src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjowvt7ARvZePh_g5Pf_00OjAVYkkiIX_SdWspqWvZBo1qL7Ixs2qM23GZcItU0BzacX7fYyaj62ipAiu5MqgqylHQyeT_7GYOrBmtgz6ZhEyv_d9EKzNjnUJCIRR2-IS5nPWLcJrctixDFZPzciKS5Zat-fpbYCOFQ2o1xfUB91aPT5e6Ys6SFB_J8fnza/s1060/LOGO%20PNG%20transparent.png" 
              alt="Logo" 
              className="h-12 w-auto object-contain"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = "https://placehold.co/120x40/ffffff/000000?text=Logo";
              }}
            />
          </div>
          <div className="w-full">
            ...
            <p className="text-[10px] text-slate-500 mt-1.5 leading-relaxed">
              ...
              <a 
                ...
                className="font-bold text-blue-400 hover:text-blue-300 transition-all duration-300 inline-block"
                style={{ textShadow: '0 0 10px rgba(96, 165, 250, 0.9), 0 0 20px rgba(96, 165, 250, 0.5)' }}
              >
                hendyka
              </a>
            </p>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-20 bg-[#111827]/80 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 lg:px-10 z-10 shrink-0">
          <div className="relative w-full max-w-md">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2.5 border border-slate-700 rounded-xl bg-slate-900/50 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
              value={searchInput}
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-lg ${
                isUploaded ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-900/20' : 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-900/20'
              }`}
            >
              {isUploaded ? <CheckCircle2 size={18} /> : <FileUp size={18} />}
              <span className="hidden sm:inline">{isUploaded ? 'Timpa Database Cloud' : 'Upload Admin CSV'}</span>
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 scroll-smooth">
          {uploadError && (
             <div className="mb-6 bg-rose-500/20 border border-rose-500/30 rounded-2xl p-4 text-sm text-rose-400 flex items-center gap-3 shadow-lg">
                <ShieldAlert size={20} />
                {uploadError}
             </div>
          )}
...
function Top10Card(props) {
  ...
  return (
    <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-5 flex flex-col h-[520px] shadow-lg">
      <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2 shrink-0 border-b border-slate-700/50 pb-3">
        {icon} {title}
      </h2>
        ...
        {data.map((item, i) => (
          <div key={i} className="bg-slate-900/60 rounded-xl border border-slate-700/50 p-4 hover:border-slate-500 transition-colors">
             <div className="flex justify-between items-start border-b border-slate-700/50 pb-2 mb-2">
                ...
                <div 
                   className={`font-mono font-bold text-sm text-right shrink-0 truncate max-w-[120px] ${colorClass}`} 
                   title={isFreeFloat ? item.publicSharesVolume.toLocaleString() : item.totalShares.toLocaleString()}
                >
             ...
             <div className="space-y-2 max-h-32 overflow-y-auto scrollbar-hide pr-1">
                {item.holdings.map((h, j) => (
                   <div key={j} className="flex justify-between items-center text-xs bg-[#0a0f1c]/50 p-1.5 rounded">
                      <span 
                        className={`font-bold w-12 shrink-0 ${isFreeFloat ? 'text-amber-400 truncate w-24' : 'text-blue-400'}`}
                        title={isFreeFloat ? h.investor : h.ticker}
                      >
                        {isFreeFloat ? h.investor : h.ticker}
                      </span>
                      <span 
                        className="text-slate-400 font-mono text-right flex-1 truncate pr-3" 
                        title={`${h.shares.toLocaleString()} lbr`}
                      >
                        {h.shares.toLocaleString()} lbr
                      </span>
                      <span 
                        className="text-slate-200 font-mono font-bold w-14 text-right shrink-0"
                ...
                {isFreeFloat && item.publicSisa > 0 && (
                   <div className="flex justify-between items-center text-xs bg-slate-800/80 p-1.5 rounded mt-1">
                      <span className="text-slate-400 font-medium truncate w-24" title="Publik Ritel (<1%)">Publik Ritel (&lt;1%)</span>
                      <span 
                        className="text-slate-500 font-mono text-right flex-1 truncate pr-3"
                        title={`${Math.round(item.publicSisaShares || 0).toLocaleString()} lbr`}
                      >
                        {Math.round(item.publicSisaShares || 0).toLocaleString()} lbr
                      </span>
                      <span className="text-emerald-400 font-mono font-bold w-14 text-right shrink-0" title={`${item.publicSisa.toFixed(2)}%`}>{item.publicSisa.toFixed(2)}%</span>
                   </div>
                )}
             </div>
          </div>
...
function DashboardView({ data }) {
  ...
  const { topIndividu, topInstitusi, topForeign, topLocal, topCountry, categoryStats, topFreeFloat } = useMemo(() => {
    ...
    const cMap = new Map();
    const emitenFFMap = new Map();
    
    const catMap = {
      'CP': 0, 'IB': 0, 'FD': 0, 'OT': 0, 'ID': 0, 'MF': 0, 'IS': 0, 'PF': 0, 'SC': 0
    };
    ...
    data.forEach(d => {
      ...
      if (!invMap.has(cleanInvestorName)) {
        invMap.set(cleanInvestorName, {
          investor: cleanInvestorName, 
          type: d.category.code,
          localForeign: d.localForeign,
          totalShares: 0,
          holdings: []
        });
      }
      const inv = invMap.get(cleanInvestorName);
      inv.totalShares += d.shares;
      inv.holdings.push({ ticker: d.ticker, shares: d.shares, percentage: d.percentage });

      const country = d.domicile ? d.domicile.trim().toUpperCase() : 'OTHERS';
      if (!cMap.has(country)) {
        cMap.set(country, { country, totalShares: 0, holdingsMap: new Map() });
      }
      const c = cMap.get(country);
      c.totalShares += d.shares;
      
      if (!c.holdingsMap.has(d.ticker)) {
        c.holdingsMap.set(d.ticker, { ticker: d.ticker, shares: 0, percentage: 0 });
      }
      const h = c.holdingsMap.get(d.ticker);
      h.shares += d.shares;
      h.percentage += d.percentage;

      let code = d.category.code;
      if (catMap[code] === undefined) code = 'OT'; 
      catMap[code] += d.shares;
      totalAllShares += d.shares;

      if (!emitenFFMap.has(d.ticker)) {
        emitenFFMap.set(d.ticker, { ticker: d.ticker, name: d.emitenName, nonPublicTotal: 0, totalSharesInCompany: 0, publicHolders: [] });
      }
      ...
      if (eFF.totalSharesInCompany === 0 && d.percentage > 0) {
          eFF.totalSharesInCompany = d.shares / (d.percentage / 100);
      }
    ...
    allInv.forEach(inv => inv.holdings.sort((a,b) => b.shares - a.shares));

    const allCountries = Array.from(cMap.values()).map(c => ({
      country: c.country,
      totalShares: c.totalShares,
      holdings: Array.from(c.holdingsMap.values()).sort((a,b) => b.shares - a.shares)
    }));

    const categoryStats = Object.keys(catMap).map(code => ({
      code,
      name: CATEGORY_NAMES[code],
      shares: catMap[code],
      percentage: totalAllShares > 0 ? (catMap[code] / totalAllShares) * 100 : 0
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 shadow-lg mb-6">
        <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2 border-b border-slate-700/50 pb-3">
          ...
          Distribusi Kepemilikan Berdasarkan Kategori
        </h2>
          ...
          {categoryStats.map(stat => (
            <div key={stat.code} className="bg-slate-900/50 rounded-xl border border-slate-700/50 p-4 hover:border-slate-500 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <span className="font-bold text-slate-300 text-sm truncate pr-2" title={stat.name}>{stat.name}</span>
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${CAT_COLOR_MAP[stat.code]} bg-opacity-20 text-white shadow-sm shrink-0`}>
                  {stat.code}
                </span>
              </div>
              ...
              <div className="text-xs text-slate-400 font-mono truncate" title={`${stat.shares.toLocaleString()} lbr`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        <Top10Card 
          title="Top 10 Public Share Float" 
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {filteredTickers.map(group => {
        ...
        return (
          <div key={group.ticker} className="bg-[#151e2f] border border-slate-800 rounded-2xl overflow-hidden shadow-lg flex flex-col xl:flex-row">
            <div className="xl:w-1/3 p-6 border-b xl:border-b-0 xl:border-r border-slate-800 bg-slate-900/30 flex flex-col justify-between">
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-400 mt-1 leading-snug">{group.name}</p>
              </div>
...
function InvestorView({ data, searchQuery }) {
  ...
  const investorGroupsArray = useMemo(() => {
    ...
    return Object.values(groups).map(g => {
      ...
      g.searchKey = `${g.investor} ${g.holdings.map(h => `${h.ticker} ${h.emitenName}`).join(' ')}`.toLowerCase();
      return g;
  ...
  const filteredInvestors = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      if (alphaFilter === '#') {
        items = items.filter(g => !/^[A-Z]/i.test(g.investor.trim()));
      } else {
        items = items.filter(g => g.investor.trim().toUpperCase().startsWith(alphaFilter));
      }
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {filteredInvestors.map(group => {
        ...
        const pieData = group.holdings.slice(0, 15).map((h, i) => ({
          label: h.ticker,
        ...
        return (
          <div key={group.investor} className="bg-[#151e2f] border border-slate-800 rounded-2xl overflow-hidden shadow-lg flex flex-col xl:flex-row">
            <div className="xl:w-1/3 p-6 border-b xl:border-b-0 xl:border-r border-slate-800 bg-slate-900/30 flex flex-col justify-between">
              <div className="mb-6">
                <h2 className="text-xl font-bold text-white leading-tight mb-3">{group.investor}</h2>
                <CategoryBadge category={group.category} isPengendali={group.isPengendali} />
              </div>
              <div className="flex flex-col items-center gap-4 flex-1 justify-center">
                <DonutChart data={pieData} size={140} />
                <div className="w-full mt-4 text-center">
                   <p className="text-xs text-slate-400 mb-1">Total Volume Saham Dilacak</p>
                   <p className="font-mono text-white font-bold">{totalVolume.toLocaleString()} lbr</p>
                </div>
              </div>
            </div>
            ...
            <div className="xl:w-2/3 p-6 overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[500px]">
                <thead>
                  <tr className="border-b border-slate-700/50 text-xs uppercase text-slate-500 tracking-wider">
                    <th className="py-3 px-2 font-medium">Emiten</th>
                    <th className="py-3 px-2 font-medium">Perusahaan</th>
                    <th className="py-3 px-2 font-medium text-right">Volume Pegang</th>
                    <th className="py-3 px-2 font-medium text-right">% Kuasa</th>
                  </tr>
                </thead>
                <tbody>
                  {group.holdings.map((holding, idx) => (
                    <tr key={holding.id} className="border-b border-slate-800/30 hover:bg-slate-800/20 transition-colors">
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: idx < 15 ? CHART_COLORS[idx % CHART_COLORS.length] : '#334155' }}></div>
                          <span className="font-bold text-indigo-400">{holding.ticker}</span>
                        </div>
                      </td>
                      <td className="py-3 px-2 text-slate-300 text-sm truncate max-w-[200px]" title={holding.emitenName}>
                        {holding.emitenName}
                      </td>
                      <td className="py-3 px-2 text-right text-slate-400 font-mono text-xs">
...
function FreeFloatView({ data, searchQuery }) {
  const [filterCategory, setFilterCategory] = useState('LOW');
  ...
  const filteredItems = useMemo(() => {
    ...
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      items = items.filter(item => item.searchKey.includes(q));
    } 
      ...
      <div className="bg-slate-800/40 border border-slate-800 rounded-2xl overflow-hidden shadow-lg">
        <div className="p-6 border-b border-slate-800 flex flex-col gap-5">
           ...
           <div className="flex flex-wrap items-center gap-2">
             ...
             <button onClick={() => setFilterCategory('ALL')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors border ${filterCategory === 'ALL' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-900/50 text-slate-400 border-slate-700 hover:bg-slate-800'}`}>Semua</button>
           </div>
        </div>
              ...
              {filteredItems.length === 0 && (
                <tr>
                  <td colSpan="5" className="py-12 text-center text-slate-500">
                    Tidak ada emiten yang sesuai dengan pencarian.
                  </td>
                </tr>
              )}
            </tbody>
...
// --- OPTIMIZED NETWORK VIEW (PHYSICS ENGINE DENGAN AUTO-SLEEP) ---
function NetworkView({ data, searchQuery }) {
  ...
  const graphData = useMemo(() => {
    ...
    // Optimasi Ekstrim: Batasi hingga 120 entitas agar physics engine tidak membakar CPU
    expandedRows = expandedRows.slice(0, 120); 
    ...
    expandedRows.forEach(r => {
      if (!nodeMap.has(r.ticker)) {
        nodeMap.set(r.ticker, { id: r.ticker, label: r.ticker, type: 'emiten', size: 18 });
      }
      if (!nodeMap.has(r.investor)) {
        nodeMap.set(r.investor, { id: r.investor, label: r.investor, type: 'investor', size: Math.max(10, Math.min(24, r.percentage / 1.5)) });
      }
  ...
  useEffect(() => {
    ...
    canvas.onmousedown = (e) => {
      ...
      draggedNode = nodes.find(n => Math.hypot(n.x - simX, n.y - simY) < n.size + 8);
      if (!draggedNode) {
        isPanning = true;
        startPanX = mx - t.x;
        startPanY = my - t.y;
    ...
    canvas.onmousemove = (e) => {
      ...
      if (draggedNode) {
        ...
        hoveredNode = nodes.find(n => Math.hypot(n.x - simX, n.y - simY) < n.size + 8);
    ...
    const simulate = () => {
      ...
      simLinks.forEach(link => {
        ...
        ctx.lineWidth = isHovered ? Math.max(2, link.value * 0.2) : Math.max(0.5, link.value * 0.08);
        ctx.strokeStyle = isHovered ? 'rgba(239, 68, 68, 1)' : 'rgba(239, 68, 68, 0.25)';
        ctx.stroke();
        ...
        if (isHovered) {
          ...
          ctx.fillText(`${link.value.toFixed(2)}%`, midX, midY - 6);
        }
      });
      ...
      nodes.forEach(n => {
        ...
        if (isMatched || isHovered) {
          ...
          ctx.shadowBlur = isMatched ? 20 : 10;
        ...
        ctx.fillStyle = isMatched || isHovered ? '#ffffff' : '#94a3b8';
      ...
      // AUTO-SLEEP TRIGGER: Stop physics calculations if nodes are settled
    ...
    simulate(); // Trigger loop pertama kali
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col pb-6">
      <div className="bg-slate-800/40 border border-slate-800 rounded-2xl p-6 shadow-lg flex-1 flex flex-col min-h-[600px]">
        <div className="mb-4">
           <h2 className="text-xl font-bold text-white flex items-center gap-2">
        ...
        {!graphData ? (
           ...
           <div className="flex-1 relative bg-[#0a0f1c] rounded-xl overflow-hidden border border-slate-700/50 shadow-inner group">
              <canvas ref={canvasRef} className="absolute inset-0 w-full h-full cursor-grab active:cursor-grabbing" />
              
              <div className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur px-3 py-2 rounded-lg border border-slate-700 text-xs text-slate-300 pointer-events-none">
                 {graphData.nodes.length} Entitas | {graphData.links.length} Koneksi
                 <br/>
                 <span className="text-blue-400 font-bold inline-block mt-1 mr-2">● Emiten</span>
              ...
              <div className="absolute bottom-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={handleZoomIn} className="p-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-lg text-slate-300 shadow-lg" title="Zoom In">
                  <ZoomIn size={20} />
                </button>
                <button onClick={handleZoomOut} className="p-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-lg text-slate-300 shadow-lg" title="Zoom Out">
                  <ZoomOut size={20} />
                </button>
                <button onClick={handleResetZoom} className="p-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-lg text-slate-300 shadow-lg" title="Reset View">
                  <Maximize size={20} />
                </button>
              </div>

              <div className="absolute bottom-4 left-4 text-[10px] text-slate-500 bg-slate-900/50 px-2 py-1 rounded pointer-events-none">
                Scroll untuk Zoom • Drag untuk Menggeser • Hover untuk Info Persentase
              </div>
           </div>
        )}
      </div>
...
function DonutChart({ data, size = 120 }) {
  ...
  const gradientStops = data.map(d => {
    const start = currentPercent.toFixed(2);
    const end = (currentPercent + d.percentage).toFixed(2);
    currentPercent += d.percentage;
    return `${d.color} ${start}% ${end}%`;
  }).join(', ');

  const gradientStyle = currentPercent > 0 
    ? `conic-gradient(${gradientStops})` 
    : '#1e293b'; 

  return (
    <div className="relative rounded-full flex items-center justify-center shrink-0 shadow-xl" 
         style={{ width: size, height: size, background: gradientStyle }}>
      <div className="bg-[#151e2f] rounded-full absolute inset-0 m-auto" 
           style={{ width: size * 0.65, height: size * 0.65 }}></div>
    </div>
  );
}
...
function AlphabetFilter({ selected, onChange }) {
  ...
  return (
    <div className="flex items-center gap-2 overflow-x-auto py-2 scrollbar-hide border-b border-slate-800/50 mb-6">
      {letters.map(l => (
        <button
          key={l}
          onClick={() => onChange(l)}
          title={l === '#' ? 'Karakter Lain / Angka' : (l === 'ALL' ? 'Tampilkan Semua Data' : '')}
          className={`shrink-0 flex items-center justify-center rounded-md text-xs font-bold transition-colors ${
            l === 'ALL' ? 'px-4 h-8 tracking-wider' : 'w-8 h-8'
          } ${
            selected === l 
...
function NavButton({ icon, label, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center space-x-3 w-full px-4 py-3 rounded-xl transition-all whitespace-nowrap md:whitespace-normal
        ${active 
...
function StatCard({ title, value, icon, color }) {
  return (
    <div className="bg-[#151e2f] border border-slate-800 p-6 rounded-2xl flex items-center justify-between shadow-lg">
      ...
      <div className={`p-4 bg-slate-800/50 border border-slate-700/50 rounded-xl ${color}`}>
        {icon}
      </div>
    </div>
  );
...
function CategoryBadge({ category, isPengendali, small }) {
  ...
  const style = CAT_COLOR_MAP[code] ? `${CAT_COLOR_MAP[code]}/10 text-${CAT_COLOR_MAP[code].split('-')[1]}-400 border-${CAT_COLOR_MAP[code].split('-')[1]}-500/20` : 'bg-slate-500/10 text-slate-400 border-slate-500/20';
  ...
  return (
    <div className="flex items-center gap-2 mt-1">
      ...
      {code === 'ID' && isPengendali && (
        <span className={`inline-flex items-center gap-1 rounded-md bg-rose-500/10 text-rose-400 border border-rose-500/20 font-bold ${sizeClass}`} title="ID Pengendali">
